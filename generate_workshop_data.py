...
# MAGIC %md
# MAGIC # Workshop Data Generator — Banco Popular Genie Code Workshop
# MAGIC
# MAGIC Genera 5 tablas sintéticas de datos bancarios de **Banco Popular (Colombia)** en `workshop.gold`.
# MAGIC Modela la operación por **8 regionales** en Colombia, con la **Libranza** como producto insignia.
...
# MAGIC
# MAGIC > Los datos son 100% sintéticos. Moneda: COP. Regulador: Superintendencia Financiera de Colombia (SFC).
...
# Presencia de Banco Popular por regional (Colombia)
REGIONS = {
    "BOG": {
        "name": "Bogotá D.C. y Cundinamarca",
        "customers": 500,
        "branches": 28,
        "cities": ["Bogotá", "Soacha", "Chía", "Zipaquirá", "Facatativá", "Fusagasugá", "Girardot"],
        "lat_range": (4.3, 5.1),
        "lon_range": (-74.4, -73.8),
        "base_portfolio": 2_600_000_000_000,   # COP
    },
    "ANT": {
        "name": "Antioquia",
        "customers": 380,
        ...
        "lat_range": (5.8, 8.5),
        "lon_range": (-76.5, -74.8),
        "base_portfolio": 2_200_000_000_000,
    },
    "CAR": {
        "name": "Costa Caribe",
        "customers": 320,
        "branches": 18,
        "cities": ["Barranquilla", "Cartagena", "Santa Marta", "Montería", "Valledupar", "Sincelejo"],
        "lat_range": (8.2, 11.5),
        "lon_range": (-75.8, -73.2),
        "base_portfolio": 1_400_000_000_000,
    },
    "VAL": {
        "name": "Occidente (Valle del Cauca)",
        "customers": 290,
        "branches": 16,
        "cities": ["Cali", "Palmira", "Buenaventura", "Tuluá", "Cartago", "Buga"],
        "lat_range": (3.0, 4.8),
        "lon_range": (-77.3, -75.7),
        "base_portfolio": 1_950_000_000_000,
    },
    "SAN": {
        "name": "Santanderes",
        "customers": 270,
        "branches": 15,
        "cities": ["Bucaramanga", "Cúcuta", "Barrancabermeja", "Floridablanca", "Girón", "Ocaña"],
        "lat_range": (6.8, 8.3),
        "lon_range": (-73.5, -72.3),
        "base_portfolio": 1_600_000_000_000,
    },
    "EJE": {
        "name": "Eje Cafetero",
        "customers": 250,
        "branches": 14,
        "cities": ["Pereira", "Manizales", "Armenia", "Dosquebradas", "Chinchiná", "La Dorada"],
        "lat_range": (4.5, 5.3),
        "lon_range": (-76.0, -75.2),
        "base_portfolio": 1_200_000_000_000,
    },
    "CEN": {
        "name": "Centro Oriente (Tolima, Huila, Boyacá, Meta)",
        "customers": 220,
        "branches": 13,
        "cities": ["Ibagué", "Neiva", "Tunja", "Villavicencio", "Yopal", "Duitama"],
        "lat_range": (2.9, 6.0),
        "lon_range": (-75.3, -71.5),
        "base_portfolio": 1_000_000_000_000,
    },
    "SUR": {
        "name": "Sur (Nariño, Cauca, Putumayo, Caquetá)",
        "customers": 170,
        "branches": 10,
        "cities": ["Pasto", "Popayán", "Mocoa", "Ipiales", "Florencia", "Tumaco"],
        "lat_range": (0.5, 3.4),
        "lon_range": (-78.0, -75.0),
        "base_portfolio": 900_000_000_000,
    },
}

SEGMENTS = ["Retail", "Oficial", "PYME", "Corporativo", "Preferente"]
SEGMENT_WEIGHTS = [0.48, 0.22, 0.16, 0.08, 0.06]
RISK_PROFILES = ["A", "B", "C", "D", "E"]
RISK_WEIGHTS = [0.32, 0.36, 0.18, 0.09, 0.05]
KYC_STATUSES = ["Vigente", "Vencido", "Pendiente"]
KYC_WEIGHTS = [0.82, 0.11, 0.07]
# Libranza es el producto insignia de Banco Popular (líder del mercado)
PRODUCT_TYPES = ["Libranza", "Consumo", "Hipotecario", "Vehiculo", "Comercial", "TarjetaCredito", "Microcredito"]
PRODUCT_WEIGHTS = [0.42, 0.18, 0.13, 0.09, 0.09, 0.06, 0.03]
TRANSACTION_TYPES = ["Débito", "Crédito", "Transferencia", "Pago"]
CHANNELS = ["Sucursal", "App", "Cajero", "Web", "Corresponsal"]
CHANNEL_WEIGHTS = [0.24, 0.36, 0.17, 0.15, 0.08]
PRODUCTS_TX = ["Cuenta_Ahorros", "Cuenta_Corriente", "Tarjeta_Credito", "CDT"]
BRANCH_TYPES = ["Sucursal", "Agencia", "Corresponsal", "Digital"]
DPD_BUCKETS = ["Al_Dia", "1-30", "31-60", "61-90", "Mayor_90"]

total_customers = sum(c["customers"] for c in REGIONS.values())
total_branches = sum(c["branches"] for c in REGIONS.values())
...
for rc, info in REGIONS.items():
    for i in range(info["customers"]):
        ...
        cid = f"BPOP-{rc}-CLI-{customer_id_counter:06d}"
        ...
        # Credit score correlacionado con el perfil de riesgo
        ...
        # Gerentes de relación por regional
        rm_names = [
            f"Ana García ({rc})", f"Carlos Rodríguez ({rc})", f"María López ({rc})",
            f"Jorge Ramírez ({rc})", f"Patricia Vargas ({rc})", f"Andrés Gómez ({rc})"
        ]
        rm = np.random.choice(rm_names)

        customer_rows.append({
            "customer_id": cid,
            "customer_name": f"Cliente {rc} {i+1:04d}",
            "segment": segment,
            "region_code": rc,
...
# 10 filas: NULL region_code
null_rc_idx = np.random.choice(len(df_customers), 10, replace=False)
df_customers.loc[null_rc_idx, "region_code"] = None
...
print(f"  Defectos DQ: 10 null region, 6 fechas futuras, 5 IDs dup, 15 segment minúsculas, 18 scores inválidos")
...
for rc, info in REGIONS.items():
    for i in range(info["branches"]):
        branch_counter += 1
        bid = f"SUC-{rc}-{branch_counter:04d}"
        city = np.random.choice(info["cities"])
        ...
        zona = np.random.choice(["Norte", "Centro", "Sur", "Oriente"])
        valid_branch_ids.append(bid)

        branch_rows.append({
            "branch_id": bid,
            "branch_name": f"Banco Popular {city} #{i+1}",
            "city": city,
            "region_code": rc,
            "region_name": info["name"],
            "branch_type": btype,
...
# 8 filas: NULL region_code
null_rc_b = np.random.choice(len(df_branches), 8, replace=False)
df_branches.loc[null_rc_b, "region_code"] = None
...
print(f"  Defectos DQ: 8 null region, 3 IDs dup, 10 branch_type minúsculas")
...
for cid in sample_customers:
    ...
    rc = cust["region_code"] if pd.notna(cust["region_code"]) else "BOG"
    segment = cust["segment"]

    # Frecuencia según segmento
    freq_per_month = {"Retail": 8, "Oficial": 10, "PYME": 15, "Corporativo": 25, "Preferente": 12}.get(segment, 8)

    # Monto base según segmento y regional (COP)
    region_mult = {"BOG": 1.4, "ANT": 1.2, "CAR": 0.9, "VAL": 1.1, "SAN": 1.0, "EJE": 0.95, "CEN": 0.85, "SUR": 0.8}
    base_amount = {"Retail": 250_000, "Oficial": 900_000, "PYME": 4_000_000,
                   "Corporativo": 40_000_000, "Preferente": 6_000_000}.get(segment, 250_000)
    base_amount *= region_mult.get(rc, 1.0)

    branch_pool = [b for b in valid_branch_ids if rc in b]
    if not branch_pool:
    ...
    for d in date_range:
        ...
        if np.random.random() < freq_per_month / 30:
            ...
            for _ in range(n_tx):
                amount = max(1000.0, np.random.lognormal(np.log(base_amount), 0.6))
                ...
                tx_rows.append({
                    "transaction_id": f"TX-{rc}-{len(tx_rows)+1:08d}",
                    "customer_id": cid,
                    "branch_id": branch,
...
orphan_cids = [f"BPOP-XX-CLI-{900000+i:06d}" for i in range(60)]
...
loan_rows = []
loan_counter = 0
AVG_LOAN_COP = 40_000_000  # ~40M COP promedio por crédito

for rc, info in REGIONS.items():
    region_customers = df_customers[df_customers["region_code"] == rc]["customer_id"].tolist()
    base_port = info["base_portfolio"]
    n_loans = int(base_port / AVG_LOAN_COP)

    for i in range(n_loans):
        loan_counter += 1
        product = np.random.choice(PRODUCT_TYPES, p=PRODUCT_WEIGHTS)
        loan_id = f"BPOP-{rc}-{product[:3].upper()}-{loan_counter:06d}"

        cid = np.random.choice(region_customers) if region_customers else f"BPOP-{rc}-CLI-000001"

        disb_date = date(2020, 1, 1) + timedelta(days=int(np.random.uniform(0, 365 * 5)))

        # Plazo según producto
        term_months = {
            "Libranza": np.random.choice([24, 36, 48, 60, 72, 84]),
            "Consumo": np.random.choice([12, 24, 36, 48, 60]),
            "Hipotecario": np.random.choice([120, 180, 240, 300]),
            "Vehiculo": np.random.choice([24, 36, 48, 60, 72]),
            "Comercial": np.random.choice([12, 24, 36, 48]),
            "TarjetaCredito": 12,
            "Microcredito": np.random.choice([12, 18, 24, 36]),
        }[product]

        maturity_date = disb_date + timedelta(days=term_months * 30)

        # Monto original según producto (COP)
        amount_ranges = {
            "Libranza": (5_000_000, 80_000_000),
            "Consumo": (2_000_000, 40_000_000),
            "Hipotecario": (60_000_000, 500_000_000),
            "Vehiculo": (20_000_000, 150_000_000),
            "Comercial": (30_000_000, 1_000_000_000),
            "TarjetaCredito": (1_000_000, 25_000_000),
            "Microcredito": (1_000_000, 15_000_000),
        }
        ...
        # La libranza tiene menor mora estructural (descuento de nómina)
        dpd_probs = {
        ...
        probs = list(dpd_probs.get(risk, dpd_probs["B"]))
        if product == "Libranza":
            # Reduce probabilidad de mora: desplaza masa hacia Al_Dia
            shift = sum(probs[1:]) * 0.35
            probs = [probs[0] + shift] + [p * 0.65 for p in probs[1:]]
        dpd_bucket = np.random.choice(DPD_BUCKETS, p=np.array(probs) / sum(probs))
        ...
        # Tasa de interés efectiva anual según producto (mercado colombiano)
        rate_ranges = {
            "Libranza": (0.14, 0.20),
            "Consumo": (0.20, 0.32),
            "Hipotecario": (0.10, 0.16),
            "Vehiculo": (0.13, 0.22),
            "Comercial": (0.12, 0.26),
            "TarjetaCredito": (0.25, 0.42),
            "Microcredito": (0.28, 0.45),
        }
...
for rc, info in REGIONS.items():
    ...
    for d in kpi_date_range:
        ...
        # NPL ratio (tasa de mora) — varía por regional y tiempo
        npl_base = {"BOG": 0.031, "ANT": 0.034, "CAR": 0.052, "VAL": 0.040,
                    "SAN": 0.045, "EJE": 0.038, "CEN": 0.048, "SUR": 0.058}.get(rc, 0.04)
...
# 6 filas: region_code = "DESCONOCIDO"
unknown_idx = np.random.choice(len(df_kpis), 6, replace=False)
df_kpis.loc[unknown_idx, "region_code"] = "DESCONOCIDO"

print(f"fact_kpis_diarios: {len(df_kpis):,} filas")
print(f"  Defectos DQ: 8 NPL>1.0, 5 portfolio=0 con desembolsos, 3 centinelas, 6 regional DESCONOCIDO")
...
print("=" * 65)
print("GENERACIÓN DE DATOS WORKSHOP BANCO POPULAR — RESUMEN")
print("=" * 65)
...
print("  dim_clientes:          10 null region, 6 fechas futuras, 5 IDs dup, 15 segment minúsculas, 18 scores inválidos")
print("  dim_sucursales:        8 null region, 3 IDs dup, 10 branch_type minúsculas")
...
print("  fact_kpis_diarios:     8 NPL>1.0, 5 portfolio=0, 3 centinelas, 6 regional DESCONOCIDO")
print()
print(f"Total defectos: ~382")
print()
print("Regionales Banco Popular incluidas:")
for rc, info in REGIONS.items():
    print(f"  {rc}: {info['name']} — {info['customers']} clientes, {info['branches']} sucursales, ${info['base_portfolio']/1e12:.2f}B COP cartera")
print("=" * 65)
